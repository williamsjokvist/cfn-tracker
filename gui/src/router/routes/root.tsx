...
const EntryForm: React.FC = () => {
  ...
  const { isLoading, setLoading } = useStatStore();
  const [oldCfns, setOldCfns] = React.useState<string[] | null>(null);
  const [isRestore, setRestore] = React.useState<boolean>(false);
  ...
  const onFormSubmit: React.FormEventHandler = (e) => {
    ...
    if (isRestore) {
      setLoading(true);
      StartTracking("", true).then(() => setLoading(false));
      return;
    }
...
export const RootPage: React.FC = () => {
  ...
  React.useEffect(() => {
    if (!isTracking) {
      ...
      ResultsJSONExist().then((doesExist: boolean) => setLastJSONExist(doesExist));
    }

    if (!isInitialized)
      IsInitialized().then(isInitialized => setInitialized(isInitialized));
  }, [isTracking, isInitialized]);

  React.useEffect(() => {
    if (isTracking && !isLoading && !isPaused)
      ...
      setHeaderText(t("pause"))

    else if (!isTracking && isInitialized && !isLoading)
            ...
            <div className="relative w-full h-full grid grid-rows-[0fr_1fr] max-w-[320px]">
              ...
              <dl className="stat-grid-item w-full mt-2 relative text-center text-lg whitespace-nowrap">
                <div className="mb-2 flex gap-4 justify-between bg-slate-50 bg-opacity-5 p-3 pb-1 rounded-xl backdrop-blur">
                  ...
                  <dd className="text-4xl font-semibold">{matchHistory.wins}</dd>
                </div>
                <div className="mb-2 flex gap-4 justify-between bg-slate-50 bg-opacity-5 p-3 pb-1 rounded-xl backdrop-blur">
                  <dt className="tracking-wide font-extralight">{t("losses")}</dt>
                  <dd className="text-4xl font-semibold">{matchHistory.losses}</dd>
                </div>
                <div className="mb-2 flex gap-4 justify-between bg-slate-50 bg-opacity-5 p-3 pb-1 rounded-xl backdrop-blur">
                  <dt className="tracking-wide font-extralight">{t("winStreak")}</dt>
                  <dd className="text-4xl font-semibold">{matchHistory.winStreak}</dd>
                </div>
                <div className="mb-2 flex gap-4 justify-between bg-slate-50 bg-opacity-5 p-3 pb-1 rounded-xl backdrop-blur">
                  <dt className="tracking-wide font-extralight">{t("lpGain")}</dt>
                  <dd className="text-4xl font-semibold">{matchHistory.lpGain > 0 && "+"} {matchHistory.lpGain}</dd>
                </div>
              </dl>
            </div>
            <div className="relative mr-4 text-center h-full grid content-between justify-items-center">
              <b className='absolute top-[30%] z-50 text-4xl'>{(matchHistory.winRate > 0) && (matchHistory.winRate + '%')}</b>
              <PieChart
