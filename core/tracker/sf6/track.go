...
import (
	...
	te "github.com/williamsjokvist/cfn-tracker/core/errorsx"
...
func (t *SF6Tracker) Start(ctx context.Context, userCode string, restore bool, pollRate time.Duration) error {
	if !t.isAuthenticated {
		...
		return te.NewError(http.StatusUnauthorized, errors.New(`tracker not authenticated`))
	}
	...
	if restore {
		...
		if err != nil {
			return te.NewError(http.StatusNotFound, fmt.Errorf(`failed to get last session: %w`, err))
		}
		t.sesh = sesh
		t.user, err = t.CFNTrackerRepository.GetUserByCode(ctx, userCode)
		if err != nil {
			return te.NewError(http.StatusNotFound, fmt.Errorf(`failed to get user: %w`, err))
		}
		trackingState := t.getTrackingStateForLastMatch()
		if trackingState == nil {
			bl, err := t.fetchBattleLog(userCode)
			if err != nil {
				return te.NewError(http.StatusInternalServerError, fmt.Errorf(`failed to fetch battle log: %w`, err))
			}
